...
import com.apsy2003.harusamki.databinding.ActivityMapBinding
        ...
        seoulOpenService
            ...
            .enqueue(object : Callback<Restaurants> {
                ...
                override fun onResponse(call: Call<Restaurants>, response: Response<Restaurants>) {
                    ...
                    showRestaurants(response.body() as Restaurants)
                }
            })
    }

    fun showRestaurants(restaurants: Restaurants) {
        val latLngBounds = LatLngBounds.Builder()
        /* 파라미터로 전달된 libraries의 SeoulPublicLibraryInfo.row에 도서관 목록이 담겨 있다.
        목록에서 반복문을 통해 하나씩 꺼낸다. */
        for (res in restaurants.SebcKoreanRestaurantsKor.row) {

            //기존 마커 세팅 코드는 삭제하고 클러스터 메니저에 데이터를 추가하는 코드만 넣어줍니다.
            clusterManager.addItem(res)
            // 첫 화면에 보여줄 범위를 정하기 위해서 아래 코드 두 줄은 남겨둡니다.
            val position = LatLng(res.XCNTS.toDouble(), res.YDNTS.toDouble())
